...
struct route_evt_t {
    ...
    // u32 sport;      // L4 source port 
    // u32	dport;      // L4 dest port 
...
//
//        // Load protocol and address
//        offset_from_ip_header = iphdr.ihl * 4;
//        l4proto      = iphdr.protocol;
//        evt->saddr[0] = iphdr.saddr;
//        evt->daddr[0] = iphdr.daddr;
//        
...
//                char* tcp_header_address = ip_header_address + offset_from_ip_header;
//                bpf_probe_read(&tcphdr, sizeof(tcphdr), tcp_header_address);
//                evt->sport = tcphdr.source;
//                evt->dport = tcphdr.dest; 			 
//                evt->sport  = be16_to_cpu(evt->sport);
//                evt->dport  = be16_to_cpu(evt->dport);
//                evt->icmptype = 100;
//			 }
//                         break;
//
...
//                char* udp_header_address = ip_header_address + offset_from_ip_header;
//                bpf_probe_read(&udphdr, sizeof(udphdr), udp_header_address);
//                evt->sport = udphdr.source;
//                evt->dport = udphdr.dest;			 
//                evt->sport  = be16_to_cpu(evt->sport);
//                evt->dport  = be16_to_cpu(evt->dport);		
//			 }
//			 break;
...
//	        
//		// Compute ICMP header address and load ICMP header
//                char* icmp_header_address = ip_header_address + offset_from_ip_header;
//		struct icmphdr icmphdr;
//		bpf_probe_read(&icmphdr, sizeof(icmphdr), icmp_header_address);
//
//		// Get ICMP info   
...
//        // Assume no option header --> fixed size header
//        struct ipv6hdr* ipv6hdr = (struct ipv6hdr*)ip_header_address;
//        offset_from_ip_header = sizeof(*ipv6hdr);
//
//        // Load protocol and address
//        bpf_probe_read(&l4proto,  sizeof(ipv6hdr->nexthdr),  (char*)ipv6hdr + offsetof(struct ipv6hdr, nexthdr));
//        bpf_probe_read(evt->saddr, sizeof(ipv6hdr->saddr),   (char*)ipv6hdr + offsetof(struct ipv6hdr, saddr));
